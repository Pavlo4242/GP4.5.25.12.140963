        ...
        viewModelScope.launch {
            ...
            try {
                ...
                val managerSettings = mutableListOf<Setting>(
                    ...
                    run { // Use a run block to parse JSON cleanly
                        val jsonString = Config.get("prepopulation_locations", "[]") as String
                        val locations = mutableListOf<com.grindrplus.manager.settings.LocationData>()
                        try {
                            val jsonArray = org.json.JSONArray(jsonString)
                            for (i in 0 until jsonArray.length()) {
                                val jsonObj = jsonArray.getJSONObject(i)
                                locations.add(com.grindrplus.manager.settings.LocationData(
                                    name = jsonObj.optString("name", ""),
                                    lat = jsonObj.optString("lat", ""),
                                    lon = jsonObj.optString("lon", "")
                                ))
                            }
                        } catch (e: org.json.JSONException) {
                            // Ignore, will use empty list
                        }
                        // Pad to 3 locations for the UI
                        while (locations.size < 3) {
                            locations.add(com.grindrplus.manager.settings.LocationData("", "", ""))
                        }

                        com.grindrplus.manager.settings.LocationListSetting(
                            id = "prepopulation_locations",
                            title = "Default Teleport Locations",
                            description = "Set up to 3 default locations for fresh installs. Leave all fields blank to use built-in defaults.",
                            locations = locations.take(3), // Ensure only 3
                            onLocationsChange = { newLocations ->
                                viewModelScope.launch {
                                    val jsonArray = org.json.JSONArray()
                                    newLocations
                                        .filter { it.name.isNotBlank() && it.lat.isNotBlank() && it.lon.isNotBlank() }
                                        .forEach { loc ->
                                            try {
                                                // Validate lat/lon are doubles
                                                loc.lat.toDouble()
                                                loc.lon.toDouble()
                                                val jsonObj = org.json.JSONObject()
                                                jsonObj.put("name", loc.name)
                                                jsonObj.put("lat", loc.lat)
                                                jsonObj.put("lon", loc.lon)
                                                jsonArray.put(jsonObj)
                                            } catch (e: NumberFormatException) {
                                                // Skip invalid entries
                                            }
                                        }
                                    Config.put("prepopulation_locations", jsonArray.toString())
                                    loadSettings()
                                }
                            }
                        )
