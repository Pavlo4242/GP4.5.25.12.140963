...
class UnlimitedProfiles : Hook(
    "Unlimited profiles",
    "Allow unlimited profiles (scrolling) & Filters"
) {
    ...
    override fun init() {
        // 1. Force filters to show all items initially
        ...
        // 3. Scrolling past the limit (Repository Hook)
        ...
        val function2Class = findClass("kotlin.jvm.functions.Function2")
        ...
        profileRepoClass.hook("getProfilesWithPhotosFlow", HookStage.AFTER) { param ->
            ...
            val profileWithPhotoConstructor =
                profileWithPhotoClass.getConstructor(profileClass, List::class.java)
                ...
                if (requestedProfileIds.size > profilesWithPhoto.size) {
                    ...
                    for (profileId in requestedProfileIds) {
                        if (profileId !in profileIdSet) {
                            ...
                            missingProfiles.add(
                                profileWithPhotoConstructor.newInstance(
                                    profile,
                                    emptyList<Any>()
                                )
                            )
        ...
        findClass(serverDrivenCascadeCachedState)
            .hook("getItems", HookStage.AFTER) { param ->
                if (!(Config.get("custom_filtering_enabled", false) as Boolean)) return@hook
    ...
    // ... (Copy the applyCustomFilters and safeGetField helper methods from your previous file here) ...
    // For brevity, I am not repeating the long filtering logic as it was correct in your provided code.
    // Just ensure applyCustomFilters is pasted inside this class.
